...
router.get('/', requireAuth, async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT
        w.watch_id, w.target_price, w.is_active, w.created_at,
        ...
        (SELECT MIN(ph2.price) FROM price_history ph2
         WHERE ph2.product_id = p.product_id) AS all_time_low,
        (SELECT ph3.price FROM price_history ph3
         WHERE ph3.product_id = p.product_id
         ORDER BY ph3.recorded_at ASC LIMIT 1) AS first_price,
        ROUND(
          100 * (
            (SELECT ph4.price FROM price_history ph4
             WHERE ph4.product_id = p.product_id
             ORDER BY ph4.recorded_at ASC LIMIT 1)
            -
            (SELECT ph5.price FROM price_history ph5
             WHERE ph5.product_id = p.product_id
             ORDER BY ph5.recorded_at DESC LIMIT 1)
          ) / NULLIF(
            (SELECT ph6.price FROM price_history ph6
             WHERE ph6.product_id = p.product_id
             ORDER BY ph6.recorded_at ASC LIMIT 1),
            0
          ),
          1
        ) AS drop_pct
...
// POST /api/watchlist
// FIXED: PostgreSQL does NOT support MySQL's "ON DUPLICATE KEY UPDATE"
// PostgreSQL uses "ON CONFLICT (columns) DO UPDATE SET ..."
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE watchlist SET target_price = ? WHERE watch_id = ? AND user_id = ?',
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    await db.query(
      'DELETE FROM watchlist WHERE watch_id = ? AND user_id = ?',
